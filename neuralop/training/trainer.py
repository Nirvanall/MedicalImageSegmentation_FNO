...
class Trainer:
    def __init__(self, model, n_epochs, wandb_log=True, device=None,
                 ...
                 log_test_interval=1, log_output=False, use_distributed=False, verbose=True, save_checkpoint=True, save_name=None):
        """
    ...
    def train(self, train_loader, test_loaders, output_encoder,
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx, sample in enumerate(train_loader):
                ...
                if regularizer:
                    loss += regularizer.loss.mean()

                loss.mean().backward()
                
                optimizer.step()
                train_err += loss.mean().item()
        
                with torch.no_grad():
                    avg_loss += loss.mean().item()
                    if regularizer:
                        avg_lasso_loss += regularizer.loss.mean()
            ...
            elif epoch % self.log_test_interval == 0: 
            ...
            if self.save_checkpoint:
                if epoch == 0:
                    msg = f'Saving the best checkpoint at [{epoch}] epoch'
                    print(msg)
                    torch.save(model, self.save_name + '-best-checkpoint.pt')
                    best_training_loss = avg_loss
                elif avg_loss < best_training_loss:
                    msg = f'Saving the best checkpoint at [{epoch}] epoch'
                    print(msg)
                    torch.save(model, self.save_name + '-best-checkpoint.pt')
                    best_training_loss = avg_loss

            self.training_loss_dict[epoch] = avg_loss
            
            
    def get_training_loss(self):
        return self.training_loss_dict
        ...
        with torch.no_grad():
            for it, sample in enumerate(data_loader):
                ...
                for loss_name, loss in loss_dict.items():
                    errors[f'{log_prefix}_{loss_name}'] += loss(out, y).mean().item()
